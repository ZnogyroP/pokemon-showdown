...
export const Moves: {[moveid: string]: MoveData} = {
	...
	flamerunner: {
		...
		self: {
			onHit(pokemon) {
				this.add('-start', target, 'typeadd', 'Fire', '[from] move: Flame Runner');
			},
		},
		secondary: null,
		target: "normal",
	...
	tearapart: {
		...
		basePower: 20,
		category: "Physical",
		shortDesc: "Hits twice; each hit lowers foe's Def.",
		name: "Tear Apart",
		pp: 15,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1},
		...
		onBasePower (basePower, pokemon, target) {
			...
			for (const condition of sideConditions) {
				if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
					...
					pokemon.side.removeSideCondition('watershield');
				}
			}
	...
	dundaboat: {
		...
		onHit(target, source, move) {
			...
			if (result === 0) {
				...
				if (user.hasType('Electric')) {
					pokemon.setType(pokemon.getTypes(true).map(type => type === "Electric" ? "???" : type));
					this.add('-start', pokemon, 'typechange', pokemon.types.join('/'), '[from] move: Dundaboat');
				}
