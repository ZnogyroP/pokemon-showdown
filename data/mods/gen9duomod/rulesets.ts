	...
	secondspin: { // FIX
		...
		if (result === 0) {
          ...
          this.boost({spe: 1}, target);
			...
			this.hint("Roulette Wheel Result 18 - duo forgot to fix this one so nothing happens");
		}
			
		else if (result === 18) { 
			this.hint("Roulette Wheel Result 19 - duo forgot to fix this one so nothing happens");
		} 			
