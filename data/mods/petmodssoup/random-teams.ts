	...
	queryMoves(moves: string[] | null, hasType: {[k: string]: boolean} = {}, hasAbility: {[k: string]: boolean} = {}, movePool: string[] = []) {
		...
		for (const [k, moveId] of moves.entries()) {
			...
			if (move.basePower > 30 || move.multihit || move.basePowerCallback || moveid === 'infestation' || moveid === 'naturepower') {
				...
				} else if (movetype === 'Normal' && (hasAbility['Aerilate'] || hasAbility['Galvanize'] || hasAbility['Pixilate'] || hasAbility['Refrigerate'] || hasAbility['Magnetic Waves'])) {
					counter['stab']++;
	...
	randomSet(species: string | Species, teamDetails: RandomTeamsTypes.TeamDetails = {}, isLead = false, isDoubles = false): RandomTeamsTypes.RandomSet {
		...
		if (abilities[1]) {
			...
			do {
				...
				} else if (ability === 'Compound Eyes' || ability === 'No Guard' || ability === 'Not Funny') {
		...
		} else if (ability === 'Gorilla Tactics' || ability === 'Fowl Behaviour' || ability === 'Imposter' || (ability === 'Magnet Pull' && hasMove['bodypress'] && !isDoubles)) {
			item = 'Choice Scarf';
		} else if (hasMove['trick'] || hasMove['switcheroo'] && !isDoubles) {
    		if (ability === 'Klutz' || ability === 'Magic Surge') {
				item = 'Assault Vest';
		...
		// Better than Leftovers
			...
			(hasMove['rapidspin'] && (ability === 'Regenerator' || ability === 'Surgeon Eye' || ability === 'Growth Veil' || !!counter['recovery']))) && !isDoubles
		) {
			item = 'Heavy-Duty Boots';
